...
func SafeKill(client *docker.Client, cid string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to get inspect docker container ID %v: %w", cid, err)
	}
	...
	if container_insp.State.Paused {
		...
		if err := client.UnpauseContainer(cid); err != nil {
			return fmt.Errorf("failed to unpause container %v.  May require manual cleanup: %w", cid, err)
		}
	}
	...
	if err := client.KillContainer(killopts); err != nil {
		return fmt.Errorf("failed to kill container %v.  May require manual cleanup: %w", cid, err)
	}
...
func SafeRemove(client *docker.Client, cid string) error {
	...
	if err := client.RemoveContainer(rmopts); err != nil {
		return fmt.Errorf("failed to remove container %v.  May require manual cleanup: %w", cid, err)
	}
