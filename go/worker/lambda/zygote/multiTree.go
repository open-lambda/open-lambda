...
func (mt *MultiTree) Create(childSandboxPool sandbox.SandboxPool, isLeaf bool, codeDir, scratchDir string, meta *sandbox.SandboxMeta) (sandbox.Sandbox, error) {
	idx := rand.Intn(len(mt.trees))
	return mt.trees[idx].Create(childSandboxPool, isLeaf, codeDir, scratchDir, meta)
}
