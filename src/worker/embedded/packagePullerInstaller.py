...
def f(event):
    ...
    pip_mirror = event["pip_mirror"]
    mirror_host = event["mirror_host"]
    install_cmds = ['pip3', 'install', '--no-deps', pkg, '--cache-dir', '/tmp/.cache', '-t', '/host/files']
    if not alreadyInstalled:
        if not pip_mirror:
            install_cmds += [f"--trusted-host={mirror_host}", f"--index-url={pip_mirror}",  "-vvv"]

        try:
            print(f"[packagePullerInstaller.py] attempting install with command: {install_cmds}")
            out = subprocess.check_output(install_cmds)
