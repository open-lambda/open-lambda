"""``olTornado.gen`` implements generator-based coroutines.
...
If ``olTornado.platform.twisted`` is imported, it is also possible to
...
from olTornado.util import TimeoutError
...
def with_timeout(
    ...
    Raises `olTornado.util.TimeoutError` if the input future does not
...
class Runner(object):
    """Internal implementation of `olTornado.gen.coroutine`.
...
def convert_yielded(yielded: _Yieldable) -> Future:
    ...
    may be extended to support additional types. For example::
        ...
        def _(asyncio_future):
            return olTornado.platform.asyncio.to_tornado_future(asyncio_future)
