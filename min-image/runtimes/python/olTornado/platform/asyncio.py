...
from olTornado.ioloop import IOLoop, _Selectable
...
def to_tornado_future(asyncio_future: asyncio.Future) -> asyncio.Future:
    """Convert an `asyncio.Future` to a `olTornado.concurrent.Future`.
...
def to_asyncio_future(tornado_future: asyncio.Future) -> asyncio.Future:
    ...
    .. versionchanged:: 4.3
       ...
       `olTornado.concurrent.Future`.

    .. deprecated:: 5.0
       Tornado ``Futures`` have been merged with `asyncio.Future`,
       so this method is now equivalent to `olTornado.gen.convert_yielded`.
    """
