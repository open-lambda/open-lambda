...
(and even that is often done indirectly via `olTornado.web.Application.listen`).

.. versionchanged:: 4.0

   The ``HTTPRequest`` class that used to live in this module has been moved
   to `olTornado.httputil.HTTPServerRequest`.  The old name remains as an alias.
"""

import socket
import ssl

from olTornado.escape import native_str
from olTornado.http1connection import HTTP1ServerConnection, HTTP1ConnectionParameters
from olTornado import httputil
from olTornado import iostream
from olTornado import netutil
from olTornado.tcpserver import TCPServer
from olTornado.util import Configurable
...
class HTTPServer(TCPServer, Configurable, httputil.HTTPServerConnectionDelegate):
    ...
    initialization methods are defined on `olTornado.tcpserver.TCPServer`):

    1. `~olTornado.tcpserver.TCPServer.listen`: single-process::
       ...
       In many cases, `olTornado.web.Application.listen` can be used to avoid
    ...
    2. `~olTornado.tcpserver.TCPServer.add_sockets`: multi-process::

            sockets = bind_sockets(8888)
            olTornado.process.fork_processes(0)
       ...
       `olTornado.process.fork_processes` to run a multi-process service with all
       ...
       sockets in some way other than `~olTornado.netutil.bind_sockets`.

       Note that when using this pattern, nothing that touches the event loop
       can be run before ``fork_processes``.

    3. `~olTornado.tcpserver.TCPServer.bind`/`~olTornado.tcpserver.TCPServer.start`:
    ...
    .. versionchanged:: 4.2
       `HTTPServer` is now a subclass of `olTornado.util.Configurable`.
