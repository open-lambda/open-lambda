...
* ``olTornado.access``: Per-request logging for Tornado's HTTP servers (and
  potentially other servers in the future)
* ``olTornado.application``: Logging of errors from application code (i.e.
  uncaught exceptions from callbacks)
* ``olTornado.general``: General-purpose logging, including any errors
  or warnings from Tornado itself.

These streams may be configured independently using the standard library's
`logging` module.  For example, you may wish to send ``olTornado.access`` logs
...
from olTornado.escape import _unicode
from olTornado.util import unicode_type, basestring_type
...
access_log = logging.getLogger("olTornado.access")
app_log = logging.getLogger("olTornado.application")
gen_log = logging.getLogger("olTornado.general")
...
class LogFormatter(logging.Formatter):
    ...
    * Robust against str/bytes encoding problems.
    ...
    `olTornado.options.parse_command_line` or `olTornado.options.parse_config_file`
...
def enable_pretty_logging(
    ...
    This is called automatically by `olTornado.options.parse_command_line`
    and `olTornado.options.parse_config_file`.
    """
    if options is None:
        import olTornado.options

        options = olTornado.options.options
