...
`olTornado.web.RequestHandler`, which load templates automatically based
...
``{% module *expr* %}``
    Renders a `~olTornado.web.UIModule`.  The output of the ``UIModule`` is
    not escaped::

        {% module Template("foo.html", arg=42) %}

    ``UIModules`` are a feature of the `olTornado.web.RequestHandler`
...
from olTornado.util import ObjectDict, exec_in, unicode_type
...
class Template(object):
    ...
    def __init__(
        ...
        :arg olTornado.template.BaseLoader loader: the `~olTornado.template.BaseLoader` responsible
