...
#[cfg(target_arch = "wasm32")]
pub fn func_call(func_name: &str, args: &json::Value) -> Result<Option<json::Value>, String> {
...
pub fn func_call<S: ToString>(func_name: S, args: &json::Value) -> Result<Option<json::Value>, String> {
    ...
    let result = proxy.get_mut().func_call(func_name, jdata);
...
#[cfg(not(target_arch = "wasm32"))]
pub fn host_call<S: ToString>(namespace: S, func_name: S, args: &json::Value) -> Result<Option<json::Value>, String> {
    let namespace = namespace.to_string();
    let func_name = func_name.to_string();
    log::debug!("Got host_call request for \"{namespace}::{func_name}\"");

    let arg_string = serde_json::to_string(args).unwrap();
    let jdata = arg_string.as_bytes().to_vec();

    let mut proxy = crate::proxy_connection::ProxyConnection::get_instance();
    let result = proxy.get_mut().host_call(namespace, func_name, jdata);
